...
// تغییر: مقدار پیش‌فرض بیمارستان به خالی تنظیم می‌شود تا گزینه "انتخاب کنید" نمایش داده شود.
$hospital_val   = $_POST["hospital"] ?? "";
$section_val    = $_POST["section"] ?? "all";
...
// بخش پردازش درخواست GET (برای Ajax دریافت بخش‌ها)
if (isset($_GET['action'])) {
    $action = $_GET['action'];
    if ($action == 'getsection' && isset($_GET['hospital'])) {
        $hospital = $_GET['hospital'];
        // اگر بیمارستان انتخاب نشده باشد، آرایه خالی برگردانده می‌شود
        if (empty($hospital)) {
            header('Content-Type: application/json');
            echo json_encode([]);
            exit;
        }
        $stmt = $conn->prepare("SELECT DISTINCT section_name, section_code FROM hospitalization_stats WHERE hospital = ? ORDER BY section_name");
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["action"])) {
    ...
    if (empty($start_date) || empty($end_date)) {
        ...
        // دریافت مقادیر انتخاب بیمارستان و بخش
        ...
        if ($section !== "all") {
            $conditions[] = "section_code = ?";
            $params[] = $section;
            $types .= "s";
        }
        // شرط تاریخ (با استفاده از LPAD برای مقایسه صحیح ماه به صورت دو رقمی)
        ...
        $query = "SELECT section_code, section_name, year,
                  ...
                  FROM hospitalization_stats
                  $where
                  GROUP BY section_code, section_name, year
        ...
        if ($_POST["action"] === "export") {
            ...
            foreach ($details as $dataRow) {
                $col = 1;
                setCellValueByColAndRow($sheet, $col++, $rowNum, $dataRow["section_code"]);
                setCellValueByColAndRow($sheet, $col++, $rowNum, $dataRow["section_name"]);
            ...
            setCellValueByColAndRow($sheet, $col++, $rowNum, ""); // ستون کد بخش
            ...
            // محاسبه مجموع کلی برای تعداد مراجعین
...
<head>
    <meta charset="UTF-8">
    <title>آمار پذیرش بیماران بستری - داشبورد مدیر</title>
...
<body>
    <div class="container-fluid">
        <div class="row">
            ...
            <div class="col-md-10">
                <?php
                $header = new Header("آمار پذیرش بیماران بستری", $_SESSION["username"]);
                echo $header->render();
                ?>
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-3">
                                ...
                                <select name="hospital" id="hospital" class="form-select">
                                    <!-- گزینه پیش‌فرض تغییر کرده به "انتخاب کنید" بدون گزینه "همه" -->
                            ...
                            <!-- انتخاب بخش (بارگذاری خودکار پس از انتخاب بیمارستان) -->
                            <div class="col-md-3">
                            ...
                            <table class="table table-bordered table-striped table-hover">
                                ...
                                <tbody>
                                    <?php foreach ($details as $row): ?>
                                        <tr>
                                            <td><?php echo htmlspecialchars($row["section_name"], ENT_QUOTES, "UTF-8"); ?></td>
    ...
    <script>
        // تابع برای بارگذاری لیست بخش‌ها بر اساس بیمارستان انتخابی
        let cachedSections = {};

        function loadSections(hospital) {
            if (!hospital) {
                $("#section").html('<option value="all">همه</option>');
                return;
            }
            if (cachedSections[hospital]) {
                $("#section").html(cachedSections[hospital]);
                return;
            }
            $.ajax({
                url: "bastari_m.php", // نام فایل اصلی (در صورت تغییر نام، به روزرسانی شود)
                method: "GET",
                data: {
                    hospital: hospital,
                    action: 'getsection'
                ...
                success: function(data) {
                    let options = '<option value="all">همه</option>';
                    $.each(data, function(index, item) {
                        options += '<option value="' + item.section_code + '">' + item.section_name + '</option>';
                    });
                    cachedSections[hospital] = options;
                    $("#section").html(options);
                },
                error: function(jqXHR, textStatus, errorThrown) {
                    console.error("خطا در دریافت اطلاعات بخش‌ها: " + textStatus, errorThrown);
                    alert("خطا در دریافت اطلاعات بخش‌ها.");
                }
            });
        }
    ...
    <!-- بارگذاری اسکریپت تقویم ماه/سال -->
...
<?php
// بستن اتصال در پایان اجرای اسکریپت
